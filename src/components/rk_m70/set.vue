...
import { uselightStore } from "@/stores/rk_m70/lightStore";
import { useSetStore } from "@/stores/setStore";
import { useKeyStore } from "@/stores/rk_m70/keyStore";
...
const getVer = () => {
    ...
    if (pid == 0x01FE) {
        axios.get('/down/work/RKWEB/firmware/M70/M70_firmware_us.json').then(response => {
            // 请求成功处理
        ...
        axios.get('/down/work/RKWEB/firmware/M70/M70_firmware_uk.json').then(response => {
            // 请求成功处理
            version.value = response.data.version
            url.value = response.data.url
            checkVer()
