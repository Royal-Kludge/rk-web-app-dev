...
import RK_M70_Page from '@/components/rk_m70/index.vue'
import { RK_M70, RK_M70_EVENT_DEFINE } from "@/keyboard/rk_m70/rk_m70";
import Meun from "@/components/rk_m70/menu.vue";
...
onMounted(async () => {
    ...
    rk_m70.value.addEventListener(RK_M70_EVENT_DEFINE.OnReportStart, reportStart, false);
    rk_m70.value.addEventListener(RK_M70_EVENT_DEFINE.OnReportFinish, reportFinish, false);
});

onBeforeUnmount(() => {
    if (rk_m70.value != undefined) {
        rk_m70.value.removeEventListener(RK_M70_EVENT_DEFINE.OnReportFinish, reportFinish, false);
        rk_m70.value.removeEventListener(RK_M70_EVENT_DEFINE.OnReportStart, reportStart, false);
    }
});
