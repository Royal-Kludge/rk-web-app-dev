...
onMounted(async () => {
  ...
  rk_r87.value.addEventListener(RK_R87_EVENT_DEFINE.OnDongleStatusChanged, dongleStatusChanged, false);
});

onBeforeUnmount(() => {
  if (rk_r87.value != undefined) {
    rk_r87.value.removeEventListener(RK_R87_EVENT_DEFINE.OnDongleStatusChanged, dongleStatusChanged, false);
  }
});

// const isKeyboardConnect = (): boolean => {
//   let isConnect = false;
//   if (keyboard.state.connectType == ConnectionType.Dongle && rk_r87.value != undefined) {
//     isConnect = state.connectState == ConnectionStatusEnum.Connected && rk_r87.value.data.donglePwd == 0x03000000 + 0x0156;
//   } else {
