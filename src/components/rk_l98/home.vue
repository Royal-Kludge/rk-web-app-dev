...
import RK_L98_Page from '@/components/rk_l98/index.vue'
import { RK_L98, RK_L98_EVENT_DEFINE } from "@/keyboard/rk_l98/rk_l98";
import Meun from "@/components/rk_l98/menu.vue";
...
onMounted(async () => {
    ...
    rk_l98.value.addEventListener(RK_L98_EVENT_DEFINE.OnReportStart, reportStart, false);
    rk_l98.value.addEventListener(RK_L98_EVENT_DEFINE.OnReportFinish, reportFinish, false);
});

onBeforeUnmount(() => {
    if (rk_l98.value != undefined) {
        rk_l98.value.removeEventListener(RK_L98_EVENT_DEFINE.OnReportFinish, reportFinish, false);
        rk_l98.value.removeEventListener(RK_L98_EVENT_DEFINE.OnReportStart, reportStart, false);
    }
});
