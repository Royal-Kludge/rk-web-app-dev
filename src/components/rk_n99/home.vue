...
import RK_N99_Page from '@/components/rk_n99/index.vue'
import { RK_N99, RK_N99_EVENT_DEFINE } from "@/keyboard/rk_n99/rk_n99";
import Meun from "@/components/rk_n99/menu.vue";
...
onMounted(async () => {
    ...
    rk_n99.value.addEventListener(RK_N99_EVENT_DEFINE.OnReportStart, reportStart, false);
    rk_n99.value.addEventListener(RK_N99_EVENT_DEFINE.OnReportFinish, reportFinish, false);
});

onBeforeUnmount(() => {
    if (rk_n99.value != undefined) {
        rk_n99.value.removeEventListener(RK_N99_EVENT_DEFINE.OnReportFinish, reportFinish, false);
        rk_n99.value.removeEventListener(RK_N99_EVENT_DEFINE.OnReportStart, reportStart, false);
    }
});
