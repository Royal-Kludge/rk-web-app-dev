...
import RK_F99_Page from '@/components/rk_f99/index.vue'
import { RK_F99, RK_F99_EVENT_DEFINE } from "@/keyboard/beiying/rk_f99/rk_f99";
import Meun from "@/components/rk_f99/menu.vue";
...
onMounted(async () => {
    ...
    rk_f99.value.addEventListener(RK_F99_EVENT_DEFINE.OnReportStart, reportStart, false);
    rk_f99.value.addEventListener(RK_F99_EVENT_DEFINE.OnReportFinish, reportFinish, false);
});

onBeforeUnmount(() => {
    if (rk_f99.value != undefined) {
        rk_f99.value.removeEventListener(RK_F99_EVENT_DEFINE.OnReportFinish, reportFinish, false);
        rk_f99.value.removeEventListener(RK_F99_EVENT_DEFINE.OnReportStart, reportStart, false);
    }
});
