...
import { useMenuStore } from "@/stores/rk_f99/menuStore";
import { storeToRefs } from "pinia";
import { onMounted, onBeforeUnmount, ref, reactive } from "vue";
import { useKeyStore } from "@/stores/rk_f99/keyStore";
import { keyboard } from "@/keyboard/beiying/keyboard";
import { uselightStore } from "@/stores/rk_f99/lightStore";
...
onMounted(() => {
  ...
  rk_f99.value.addEventListener(RK_F99_EVENT_DEFINE.OnDongleStatusChanged, dongleStatusChanged, false);
  rk_f99.value.addEventListener(RK_F99_EVENT_DEFINE.OnPasswordGotten, passwordGotten, false);
  isShowTFT.value = keyboard.state.connectType == ConnectionType.USB;
});

onBeforeUnmount(() => {
    if (rk_f99.value != undefined) {
        rk_f99.value.removeEventListener(RK_F99_EVENT_DEFINE.OnDongleStatusChanged, dongleStatusChanged, false);
        rk_f99.value.removeEventListener(RK_F99_EVENT_DEFINE.OnPasswordGotten, passwordGotten, false);
    }
});
