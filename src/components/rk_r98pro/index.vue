...
onMounted(async () => {
  ...
  rk_r98.value.addEventListener(RK_R98_EVENT_DEFINE.OnDongleStatusChanged, dongleStatusChanged, false);
});

onBeforeUnmount(() => {
  if (rk_r98.value != undefined) {
    rk_r98.value.removeEventListener(RK_R98_EVENT_DEFINE.OnDongleStatusChanged, dongleStatusChanged, false);
  }
});
