...
onBeforeUnmount(() => {
    if (mouse != undefined && mouse.report != undefined) {
        mouse.report.removeEventListener(RK_MOUSE_EVENT_DEFINE.OnDpiLevelChanged, dpiLevelChanged, false);
    }
});

const dpiLevelChanged = async (event: any) => {
    (mouse.protocol as RK_K3).data?.led?.setDpiLevel(event.detail as number, state.value.maxDpiLevel);
    useSpeed.refresh();
};
</script>