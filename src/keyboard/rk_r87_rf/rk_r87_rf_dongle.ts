...
import { RK_R87_RF, COMMAND_ID, RK_R87_RF_EVENT_DEFINE } from './rk_r87_rf';
...
export class RK_R87_RF_Dongle extends RK_R87_RF {
    ...
    static async create(state: KeyboardState, device: HIDDevice) {
        return new RK_R87_RF_Dongle(state, device);
    }
    ...
    async init(): Promise<void> {
        ...
        dongleWorker.onmessage = (async (event: any) => {
            if (event.data == 'finish') {
                this.lastRpt = '';
                this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnReportFinish, { detail: event.data }));
    ...
    async activelyReport(data: DataView): Promise<void> {
        ...
        switch (id) {
            case 0x02:
                ...
                this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnDongleStatusChanged, { detail: this.state.ConnectionStatus }));
                break;
        }
    }
    ...
    private dongleStatusReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnDongleStatusChanged, { detail: status }));
    ...
    private passwordReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnPasswordGotten, { detail: password }));
    }

    private getProfileReport(event: any) {
        this.data.boardProfile = event.detail as BoardProfile;
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnProfileGotten, { detail: this.data.boardProfile }));
    }

    private getLedEffectReport(event: any) {
        this.data.ledEffect = event.detail as LedEffect;
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnLedEffectGotten, { detail: this.data.ledEffect }));
    }
    ...
    private getKeyMatrixReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnKeyMatrixGotten, { detail: this.data.keyMatrixs }));
    }

    private getMacrosReport(event: any) {
        this.data.macros = event.detail as Macros;
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnMacrosGotten, { detail: this.data.macros }));
    }

    private getLedColorsReport(event: any) {
        this.data.ledColors = event.detail as LedColors;
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnLedColorsGotten, { detail: this.data.ledColors }));
    }

    private setFactoryReport(event: any) {
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnSetFactorySuccess, { detail: true }));
    }

    private async nextReport(event: any) {
        //let pkt = event.detail as Packet_Dongle;
        this.dispatchEvent(new CustomEvent(RK_R87_RF_EVENT_DEFINE.OnReportStart, { detail: true }));
