...
import { RK_M87, COMMAND_ID, RK_M87_EVENT_DEFINE } from './rk_m87';
...
export class RK_M87_Dongle extends RK_M87 {
    ...
    static async create(state: KeyboardState, device: HIDDevice) {
        return new RK_M87_Dongle(state, device);
    }
    ...
    async activelyReport(data: DataView): Promise<void> {
        ...
        switch (id) {
            case 0x02:
                ...
                this.dispatchEvent(new CustomEvent(RK_M87_EVENT_DEFINE.OnDongleStatusChanged, { detail: this.state.ConnectionStatus }));
                break;
        }
    }
    ...
    private dongleStatusReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_M87_EVENT_DEFINE.OnDongleStatusChanged, { detail: status }));
    ...
    private passwordReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_M87_EVENT_DEFINE.OnPasswordGotten, { detail: password }));
    }
