...
import { RK_S98, COMMAND_ID, RK_S98_EVENT_DEFINE } from './rk_s98';
...
export class RK_S98_Dongle extends RK_S98 {
    ...
    static async create(state: KeyboardState, device: HIDDevice) {
        return new RK_S98_Dongle(state, device);
    }
    ...
    async init(): Promise<void> {
        ...
        dongleWorker.onmessage = (async (event: any) => {
            if (event.data == 'finish') {
                this.lastRpt = '';
                this.dispatchEvent(new CustomEvent(RK_S98_EVENT_DEFINE.OnReportFinish, { detail: event.data }));
    ...
    async activelyReport(data: DataView): Promise<void> {
        ...
        switch (id) {
            case 0x02:
                ...
                this.dispatchEvent(new CustomEvent(RK_S98_EVENT_DEFINE.OnDongleStatusChanged, { detail: this.state.ConnectionStatus }));
                break;
        }
    }
    ...
    private dongleStatusReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_S98_EVENT_DEFINE.OnDongleStatusChanged, { detail: status }));
    ...
    private passwordReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_S98_EVENT_DEFINE.OnPasswordGotten, { detail: password }));
    }
    ...
    private setFactoryReport(event: any) {
        this.dispatchEvent(new CustomEvent(RK_S98_EVENT_DEFINE.OnSetFactorySuccess, { detail: true }));
    }
    ...
    private async nextReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_S98_EVENT_DEFINE.OnReportStart, { detail: true }));
