...
import { RK_L75, COMMAND_ID, RK_L75_EVENT_DEFINE } from './rk_l75';
...
export class RK_L75_Dongle extends RK_L75 {
    ...
    static async create(state: KeyboardState, device: HIDDevice) {
        return new RK_L75_Dongle(state, device);
    }
    ...
    async activelyReport(data: DataView): Promise<void> {
        ...
        switch (id) {
            case 0x02:
                ...
                this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnDongleStatusChanged, { detail: this.state.ConnectionStatus }));
                break;
        }
    }
    ...
    private dongleStatusReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnDongleStatusChanged, { detail: status }));
    ...
    private passwordReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnPasswordGotten, { detail: password }));
    }
    ...
    private getLedEffectReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnLedEffectGotten, { detail: this.data.ledEffect }));
    }
    ...
    private getKeyMatrixReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnKeyMatrixGotten, { detail: this.data.keyMatrixs }));
    }

    private getMacrosReport(event: any) {
        this.data.macros = event.detail as Macros;
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnMacrosGotten, { detail: this.data.macros }));
    }

    private getLedColorsReport(event: any) {
        this.data.ledColors = event.detail as LedColors;
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnLedColorsGotten, { detail: this.data.ledColors }));
    }

    private setFactoryReport(event: any) {
        this.dispatchEvent(new CustomEvent(RK_L75_EVENT_DEFINE.OnSetFactorySuccess, { detail: true }));
    }
