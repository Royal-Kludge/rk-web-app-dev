...
import { CONFIG_TABLE_DATA } from './configTable';
...
export class Profile {
    ...
    constructor(name: string) {
        ...
        this.configTable = new Uint8Array(CONFIG_TABLE_DATA.buffer.byteLength);
        for (index = 0; index < CONFIG_TABLE_DATA.buffer.byteLength; index++) {
            this.configTable[index] = CONFIG_TABLE_DATA[index];
        }
    }
    ...
    init(defaultName: string) {
        ...
        if (tmp != null && tmp.list.length > 0 && tmp.version != undefined && tmp.version == VERSION) {
            for (let m of tmp.list) {
                if (m.keyTable != undefined && m.configTable != undefined) {
                    let tm = new Profile(m.name)
                    tm.isDefault = m.isDefault;
                    tm.keyTable = new Uint8Array(Object.values(m.keyTable));
                    tm.configTable = new Uint8Array(Object.values(m.configTable));
