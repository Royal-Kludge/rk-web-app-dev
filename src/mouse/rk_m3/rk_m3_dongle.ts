...
// import { Packet_Dongle_Block_Set } from './packets/setPacket';

import { COMMAND_ID, RK_M3, RK_M3_EVENT_DEFINE } from './rk_m3';
...
export class RK_M3_Dongle extends RK_M3 {
    ...
    static async create(state: MouseState, device: HIDDevice) {
        return new RK_M3_Dongle(state, device);
    }
    ...
    async init(): Promise<void> {
        ...
        dongleWorker.onmessage = (async (event: any) => {
            if (event.data == 'finish' || event.data == 'timeout') {
                this.dispatchEvent(new CustomEvent(RK_M3_EVENT_DEFINE.OnReportFinish, { detail: event.data }));
            } else {
    ...
    async activelyReport(data: DataView): Promise<void> {
        ...
        switch (id) {
            case 0x02:
                ...
                //this.dispatchEvent(new CustomEvent(RK_M3_EVENT_DEFINE.OnDongleStatusChanged, { detail: this.state.ConnectionStatus }));
                break;
        }
    }
    ...
    private async nextReport(event: any) {
        ...
        this.dispatchEvent(new CustomEvent(RK_M3_EVENT_DEFINE.OnReportStart, { detail: true }));
