...
import { BoardProfile, FieldEnum } from '@/keyboard/beiying/rk_s98/boardProfile';
...
import { ps } from '@/keyboard/beiying/rk_s98/profiles';

export const uselightStore = defineStore('lightinfo_rk_s98', () => {
    ...
    const init = async () => {
        ...
        if (rk_s98.value != undefined && !isInited.value) {
            rk_s98.value.addEventListener(RK_S98_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_s98.value.addEventListener(RK_S98_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_s98.value.addEventListener(RK_S98_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_s98.value.data.boardProfile
            ledEffect.value = rk_s98.value.data.ledEffect
            ledColors.value = rk_s98.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_s98.value != undefined) {
            rk_s98.value.removeEventListener(RK_S98_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_s98.value.removeEventListener(RK_S98_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_s98.value.removeEventListener(RK_S98_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_s98.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_s98.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_s98.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_s98.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_s98.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_s98.value?.setLedColors(profileIndex.value);
            }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_s98.value != undefined) {
            ...
            await rk_s98.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
    ...
    const onPicked = async () => {
        ...
        if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            if (ledEffect.value != undefined) {
                await rk_s98.value?.setLedEffect(profileIndex.value);
            }
        }
        //}
    };
