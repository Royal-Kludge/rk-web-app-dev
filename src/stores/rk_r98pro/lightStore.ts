...
export const uselightStore = defineStore('lightinfo_rk_r98pro', () => {
    ...
    const init = async () => {
        ...
        if (rk_r98pro.value != undefined && !isInited.value) {
            rk_r98pro.value.addEventListener(RK_R98_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_r98pro.value.addEventListener(RK_R98_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_r98pro.value.addEventListener(RK_R98_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_r98pro.value.data.boardProfile
            ledEffect.value = rk_r98pro.value.data.ledEffect
            ledColors.value = rk_r98pro.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_r98pro.value != undefined) {
            rk_r98pro.value.removeEventListener(RK_R98_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_r98pro.value.removeEventListener(RK_R98_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_r98pro.value.removeEventListener(RK_R98_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_r98pro.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_r98pro.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_r98pro.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_r98pro.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_r98pro.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_r98pro.value?.setLedColors(profileIndex.value);
            }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_r98pro.value != undefined) {
            ...
            await rk_r98pro.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
    ...
    const onPicked = async () => {
        ...
        if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            if (ledEffect.value != undefined) {
                await rk_r98pro.value?.setLedEffect(profileIndex.value);
            }
        }
        //}
    };
