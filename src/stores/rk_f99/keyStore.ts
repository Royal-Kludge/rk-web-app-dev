...
import { RK_F99, RK_F99_EVENT_DEFINE } from '@/keyboard/beiying/rk_f99/rk_f99';
...
import { KeyMatrix } from '@/keyboard/beiying/rk_f99/keyMatrix';
import { Action, Macro, Macros } from '@/keyboard/beiying/rk_f99/macros';
import { Profile, ps } from '@/keyboard/beiying/rk_f99/profiles';
import { KeyCodeMap } from '@/common/keyCode'
import fileSaver from "file-saver";
import { ElMessage } from 'element-plus'

import { storage } from '@/common/storage';
import { BoardProfile, FieldEnum, PROFILE_DEFAULT_DATA } from '@/keyboard/beiying/rk_f99/boardProfile';
import { LedEffect } from '@/keyboard/beiying/rk_f99/ledEffect';
import { LedColors } from '@/keyboard/beiying/rk_f99/ledColors';
import { useI18n } from 'vue-i18n';

export const useKeyStore = defineStore('keyinfo_rk_f99', () => {
  ...
  const keyMatrix_us = [
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_F1, style: 'key', index: getIndex(0, 1), keyData: getKeyData(getIndex(0, 1)) },
        ...
        { key: KeyDefineEnum.KEY_F5, style: 'key', index: getIndex(0, 5), keyData: getKeyData(getIndex(0, 5)) },
        ...
        { key: KeyDefineEnum.KEY_F9, style: 'key', index: getIndex(0, 9), keyData: getKeyData(getIndex(0, 9)) },
        ...
        { key: KeyDefineEnum.KEY_PRINT, style: 'key', index: getIndex(0, 13), keyData: getKeyData(getIndex(0, 13)) },
        { key: KeyDefineEnum.KEY_DEL, style: 'key', index: getIndex(0, 14), keyData: getKeyData(getIndex(0, 14)) },
        { key: KeyDefineEnum.KEY_HOME, style: 'key space-l', index: getIndex(0, 15), keyData: getKeyData(getIndex(0, 15)) },
        { key: KeyDefineEnum.KEY_INS, style: 'key', index: getIndex(0, 16), keyData: getKeyData(getIndex(0, 16)) },
        { key: KeyDefineEnum.KEY_PGUP, style: 'key', index: getIndex(0, 17), keyData: getKeyData(getIndex(0, 17)) },
        { key: KeyDefineEnum.KEY_PGDN, style: 'key', index: getIndex(0, 18), keyData: getKeyData(getIndex(0, 18)) },
      ]
    },
    {
      line: 2,
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.CTRL_R, style: 'key', index: getIndex(5, 12), keyData: getKeyData(getIndex(5, 12)) },
        { key: KeyDefineEnum.KEY_LeftArrow, style: 'key space-l', index: getIndex(5, 13), keyData: getKeyData(getIndex(5, 13)) },
  ...
  const collapsedFunKeys: Record<string, Array<KeyDefineEnum>> = {
    "RK-F99": [KeyDefineEnum.KEY_CODE131, KeyDefineEnum.KEY_CODE132, KeyDefineEnum.KEY_CODE133, KeyDefineEnum.KEY_CODE14],
  ...
  const init = async () => {
    ...
    if (rk_f99.value == undefined) {
      rk_f99.value = (keyboard.protocol as RK_F99);
      let index: any;
    ...
    if (rk_f99.value != undefined && !isInited.value) {
      rk_f99.value.addEventListener(RK_F99_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
      macros.value = rk_f99.value.data.macros;
      isInited.value = true;
    }
  };
  ...
  const destroy = () => {
    if (rk_f99.value != undefined) {
      rk_f99.value.removeEventListener(RK_F99_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
    }
  ...
  const getProfiles = () => {
    if (rk_f99.value != undefined) {
      ...
      if (keyboard.keyboardDefine != undefined) {
        ...
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          ...
          for (index in keyboard.keyboardDefine.keyMatrixLayer) {
            ...
            if (!rk_f99.value.data.keyMatrixs.hasOwnProperty(table)) {
              rk_f99.value.data.keyMatrixs[table] = {};
            }
            rk_f99.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(new Uint8Array(Object.values(profile.value.get(table, layer))).buffer));
      ...
      if (profile.value.profile != undefined) {
        if (rk_f99.value.data.boardProfile != undefined) {
          rk_f99.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
        } else {
          rk_f99.value.data.boardProfile = new BoardProfile(new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer));
        }
      }

      if (profile.value.ledEffect != undefined) {
        if (rk_f99.value.data.ledEffect != undefined) {
          rk_f99.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
        } else {
          rk_f99.value.data.ledEffect = new LedEffect(new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer));
        }
      }

      if (profile.value.ledColors != undefined) {
        if (rk_f99.value.data.ledColors != undefined) {
          rk_f99.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer);
        } else {
          rk_f99.value.data.ledColors = new LedColors(new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer));
        }
      }

      KeyMatrixData.value = rk_f99.value.data.keyMatrixs;
    }
    refresh();
  }
  ...
  const clickProfile = async (obj: Profile) => {
    ...
    if (profile.value != undefined && keyboard.keyboardDefine != undefined) {
      ...
      for (type in keyboard.keyboardDefine.keyMatrixTable) {
        ...
        for (index in keyboard.keyboardDefine.keyMatrixLayer) {
          ...
          await rk_f99.value?.setKeyMatrix(layer, table, 0);
        }
      }
      // let table = MatrixTable.WIN;
      // for (index in keyboard.keyboardDefine.keyMatrixLayer) {
    ...
    if (profile.value?.profile != undefined && rk_f99.value != undefined && rk_f99.value.data.boardProfile != undefined) {
      rk_f99.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
    }
    if (profile.value?.ledEffect != undefined && rk_f99.value != undefined && rk_f99.value.data.ledEffect != undefined) {
      rk_f99.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
    }
    if (profile.value?.ledColors != undefined && rk_f99.value != undefined && rk_f99.value.data.ledColors != null) {
      rk_f99.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value?.ledColors)).buffer);
    }
  ...
  const saveProfile = () => {
    ...
    if (rk_f99.value?.data.boardProfile != undefined) {
      const dataView = new DataView(rk_f99.value?.data.boardProfile.buffer.buffer)
      profile.value?.setProfile(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))
    }
    if (rk_f99.value?.data.ledEffect != undefined) {
      const dataView = new DataView(rk_f99.value?.data.ledEffect.buffer.buffer)
      profile.value?.setledEffect(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))

    }
    if (rk_f99.value?.data.ledColors != undefined) {
      const dataView = new DataView(rk_f99.value?.data.ledColors.buffer.buffer)
  ...
  const setToFactory = async () => {
    if (rk_f99.value != undefined) {
      ...
      await rk_f99.value.setFactory();
      storage.clear();
      getProfiles();
      await rk_f99.value.setProfile(0);
      if (keyboard.keyboardDefine != undefined) {
        let index: any, type: any;
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          let table = keyboard.keyboardDefine.keyMatrixTable[type];
  ...
  const keyClick = async(index: number) => {
    ...
    if (keyCode != undefined) {
      ...
      await rk_f99.value?.setKeyMatrix(keyMatrixLayer.value, keyMatrixTable.value, 0);
  ...
  const mapping = (keyCode: KeyDefineEnum, table: MatrixTable) => {
    ...
    if (key != undefined) {
      ...
      KeyMatrixData.value[keyMatrixTable.value][keyMatrixLayer.value]?.setKeyMapping(key.index, key.KeyData.keyMappingData);
      rk_f99.value?.setKeyMatrix(keyMatrixLayer.value, keyMatrixTable.value, 0);
      unSelected();
      setUnselected(keyCode);
    }
  ...
  const keySetMacro = (index: number) => {
    ...
    if (rk_f99.value != undefined && macros.value == undefined) {
      macros.value = rk_f99.value.data.macros;
    }
