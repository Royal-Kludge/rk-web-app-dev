...
import { Macro, Macros, Action, ActionType } from '@/keyboard/beiying/rk_f99/macros';
...
export const useMacroStore = defineStore("macrostore_rk_f99", () => {
  ...
  const init = async () => {
    ...
    if (rk_f99.value != undefined && !isInited.value) {
      rk_f99.value.addEventListener(RK_F99_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_f99.value != undefined) {
      rk_f99.value.removeEventListener(RK_F99_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
  ...
  const getMacroData = async () => {
    if (rk_f99.value != undefined) {
      await rk_f99.value.getMacros();
    }
  }

  const setMacroData = async () => {
    if (rk_f99.value != undefined) {
      await rk_f99.value.setMacros();
    }
  }
