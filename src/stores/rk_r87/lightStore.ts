...
import { BoardProfile, FieldEnum } from '@/keyboard/beiying/rk_r87/boardProfile';
...
import { ps } from '@/keyboard/beiying/rk_r87/profiles';
...
export const uselightStore = defineStore('lightinfo', () => {
    ...
    const init = async () => {
        ...
        if (rk_r87.value != undefined && !isInited.value) {
            rk_r87.value.addEventListener(RK_R87_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_r87.value.addEventListener(RK_R87_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_r87.value.addEventListener(RK_R87_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_r87.value.data.boardProfile
            ledEffect.value = rk_r87.value.data.ledEffect
            ledColors.value = rk_r87.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_r87.value != undefined) {
            rk_r87.value.removeEventListener(RK_R87_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_r87.value.removeEventListener(RK_R87_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_r87.value.removeEventListener(RK_R87_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_r87.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_r87.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_r87.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_r87.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_r87.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        // if (rk_r87.value != undefined) {
        //     profile.value = rk_r87.value.data.profile
        //     ledEffect.value = rk_r87.value.data.ledEffect
        //     ledColors.value = rk_r87.value.data.ledColors
        // }
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_r87.value?.setLedColors(profileIndex.value);
            }
            // } else if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            //     if (ledEffect.value != undefined) {
            //         await rk_r87.value?.setLedEffect(profileIndex.value);
            //     }
        }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_r87.value != undefined) {
            ...
            await rk_r87.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
