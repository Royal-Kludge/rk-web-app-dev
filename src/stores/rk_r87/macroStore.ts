...
import { Macro, Macros, Action, ActionType } from '@/keyboard/beiying/rk_r87/macros';
...
export const useMacroStore = defineStore("macrostore", () => {
  ...
  const init = async () => {
    ...
    if (rk_r87.value != undefined && !isInited.value) {
      rk_r87.value.addEventListener(RK_R87_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_r87.value != undefined) {
      rk_r87.value.removeEventListener(RK_R87_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
