...
import { BoardProfile, FieldEnum } from '@/keyboard/rk_m65/boardProfile';
...
import { ps } from '@/keyboard/rk_m65/profiles';

export const uselightStore = defineStore('lightinfo_rk_m65', () => {
    ...
    const init = async () => {
        ...
        if (rk_m65.value != undefined && !isInited.value) {
            rk_m65.value.addEventListener(RK_M65_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_m65.value.addEventListener(RK_M65_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_m65.value.addEventListener(RK_M65_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_m65.value.data.boardProfile
            ledEffect.value = rk_m65.value.data.ledEffect
            ledColors.value = rk_m65.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_m65.value != undefined) {
            rk_m65.value.removeEventListener(RK_M65_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_m65.value.removeEventListener(RK_M65_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_m65.value.removeEventListener(RK_M65_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_m65.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_m65.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_m65.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_m65.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_m65.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        // if (rk_m65.value != undefined) {
        //     profile.value = rk_m65.value.data.profile
        //     ledEffect.value = rk_m65.value.data.ledEffect
        //     ledColors.value = rk_m65.value.data.ledColors
        // }
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_m65.value?.setLedColors(profileIndex.value);
            }
            // } else if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            //     if (ledEffect.value != undefined) {
            //         await rk_m65.value?.setLedEffect(profileIndex.value);
            //     }
        }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_m65.value != undefined) {
            ...
            await rk_m65.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
