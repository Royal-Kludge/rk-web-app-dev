...
import { Macro, Macros, Action, ActionType } from '@/keyboard/rk_m65/macros';
...
export const useMacroStore = defineStore("macrostore_rk_m65", () => {
  ...
  const init = async () => {
    ...
    if (rk_m65.value != undefined && !isInited.value) {
      rk_m65.value.addEventListener(RK_M65_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_m65.value != undefined) {
      rk_m65.value.removeEventListener(RK_M65_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
