...
import { BoardProfile, FieldEnum } from '@/keyboard/beiying/rk_n99/boardProfile';
...
import { ps } from '@/keyboard/beiying/rk_n99/profiles';

export const useLightStore = defineStore('lightinfo_rk_n99', () => {
    ...
    const init = async () => {
        ...
        if (rk_n99.value != undefined && !isInited.value) {
            rk_n99.value.addEventListener(RK_N99_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_n99.value.addEventListener(RK_N99_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_n99.value.addEventListener(RK_N99_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_n99.value.data.boardProfile
            ledEffect.value = rk_n99.value.data.ledEffect
            ledColors.value = rk_n99.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_n99.value != undefined) {
            rk_n99.value.removeEventListener(RK_N99_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_n99.value.removeEventListener(RK_N99_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_n99.value.removeEventListener(RK_N99_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_n99.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_n99.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_n99.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_n99.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_n99.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        ...
        //     profile.value = rk_l98.value.data.profile
        //     ledEffect.value = rk_l98.value.data.ledEffect
        //     ledColors.value = rk_l98.value.data.ledColors
        // }
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_n99.value?.setLedColors(profileIndex.value);
            }
            // } else if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            //     if (ledEffect.value != undefined) {
            //         await rk_l98.value?.setLedEffect(profileIndex.value);
            //     }
        }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_n99.value != undefined) {
            ...
            await rk_n99.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
    ...
    const onPicked = async () => {
        ...
        //     if (state.lightProps.light == LightEffectEnum.SelfDefine) {
        //         if (ledColors.value != undefined) {
        //             await rk_l98.value.setLedColors(profileIndex.value);
        //         }
        //     } else if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
        //         if (ledEffect.value != undefined) {
        //             await rk_l98.value.setLedEffect(profileIndex.value);
        ...
        if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            if (ledEffect.value != undefined) {
                await rk_n99.value?.setLedEffect(profileIndex.value);
            }
        }
        //}
    };
