...
import { KeyCodeEnum, KeyDefineEnum, KeyText, KeyText_Mac } from '@/common/keyCode_n99'
...
import { KeyMatrix } from '@/keyboard/beiying/rk_n99/keyMatrix';
import { Action, Macro, Macros } from '@/keyboard/beiying/rk_n99/macros';
import { Profile, ps } from '@/keyboard/beiying/rk_n99/profiles';
import { KeyCodeMap } from '@/common/keyCode_n99'
import fileSaver from "file-saver";
import { ElMessage } from 'element-plus'

import { storage } from '@/common/storage';
import { BoardProfile, FieldEnum, PROFILE_DEFAULT_DATA } from '@/keyboard/beiying/rk_n99/boardProfile';
import { LedEffect } from '@/keyboard/beiying/rk_n99/ledEffect';
import { LedColors } from '@/keyboard/beiying/rk_n99/ledColors';
import { useI18n } from 'vue-i18n';

export const useKeyStore = defineStore('keyinfo_rk_n99', () => {
  ...
  const keyMatrix_us = [
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_F1, style: 'key space-l4', index: getIndex(0, 2), keyData: getKeyData(getIndex(0, 2))},
        ...
        { key: KeyDefineEnum.KEY_F4, style: 'key', index: getIndex(0, 5), keyData: getKeyData(getIndex(0, 5)) },
        { key: KeyDefineEnum.KEY_F5, style: 'key space-l2', index: getIndex(0, 6), keyData: getKeyData(getIndex(0, 6)) },
        { key: KeyDefineEnum.KEY_F6, style: 'key', index: getIndex(0, 7), keyData: getKeyData(getIndex(0, 7)) },
        { key: KeyDefineEnum.KEY_F7, style: 'key', index: getIndex(0, 8), keyData: getKeyData(getIndex(0, 8)) },
        { key: KeyDefineEnum.KEY_F8, style: 'key', index: getIndex(0, 9), keyData: getKeyData(getIndex(0, 9)) },
        { key: KeyDefineEnum.KEY_F9, style: 'key space-l2', index: getIndex(0, 10), keyData: getKeyData(getIndex(0, 10)) },
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_INS, style: 'key space-l', index: getIndex(1, 15), keyData: getKeyData(getIndex(1, 15)) },
        { key: KeyDefineEnum.KEY_NUMLOCK, style: 'key space-l', index: getIndex(1, 16), keyData: getKeyData(getIndex(1, 16)) },
        { key: KeyDefineEnum.KEY_NUM_MINUS, style: 'key', index: getIndex(1, 17), keyData: getKeyData(getIndex(1, 17)) },
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_PGUP, style: 'key space-l', index: getIndex(2, 15), keyData: getKeyData(getIndex(2, 15)) },
        { key: KeyDefineEnum.KEY_NUM_7, style: 'key space-l', index: getIndex(2, 16), keyData: getKeyData(getIndex(2, 16)) },
        { key: KeyDefineEnum.KEY_NUM_8, style: 'key', index: getIndex(2, 17), keyData: getKeyData(getIndex(2, 17)) },
        { key: KeyDefineEnum.KEY_NUM_9, style: 'key', index: getIndex(2, 18), keyData: getKeyData(getIndex(2, 18)) },
        { key: KeyDefineEnum.KEY_NUM_PLUS, style: 'key key10', index: getIndex(2, 19), keyData: getKeyData(getIndex(2, 19)) },
      ]
    },
    {
      line: 4,
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_PGDN, style: 'key space-l', index: getIndex(3, 15), keyData: getKeyData(getIndex(3, 15)) },
        { key: KeyDefineEnum.KEY_NUM_4, style: 'key space-l', index: getIndex(3, 16), keyData: getKeyData(getIndex(3, 16)) },
        { key: KeyDefineEnum.KEY_NUM_5, style: 'key', index: getIndex(3, 17), keyData: getKeyData(getIndex(3, 17)) },
        { key: KeyDefineEnum.KEY_NUM_6, style: 'key', index: getIndex(3, 18), keyData: getKeyData(getIndex(3, 18)) },
      ]
    },
    {
      line: 5,
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_UpArrow, style: 'key space-l', index: getIndex(4, 14), keyData: getKeyData(getIndex(4, 14)) },
        { key: KeyDefineEnum.KEY_NUM_1, style: 'key space-l3', index: getIndex(4, 16), keyData: getKeyData(getIndex(4, 16)) },
        { key: KeyDefineEnum.KEY_NUM_2, style: 'key', index: getIndex(4, 17), keyData: getKeyData(getIndex(4, 17)) },
        { key: KeyDefineEnum.KEY_NUM_3, style: 'key', index: getIndex(4, 18), keyData: getKeyData(getIndex(4, 18)) },
        { key: KeyDefineEnum.KEY_NUM_ENTER, style: 'key key10', index: getIndex(4, 19), keyData: getKeyData(getIndex(4, 19))},
      ]
    },
    {
      line: 6,
      ...
      keys: [
        ...
        { key: KeyDefineEnum.ALT_R, style: 'key', index: getIndex(5, 8), keyData: getKeyData(getIndex(5, 8)) },
        { key: KeyDefineEnum.KEY_Fn1, style: 'key', index: getIndex(5, 9), keyData: getKeyData(getIndex(5, 9)) },
        { key: KeyDefineEnum.KEY_R_CTRL, style: 'key', index: getIndex(5, 10), keyData: getKeyData(getIndex(5, 10)) },
        { key: KeyDefineEnum.KEY_LeftArrow, style: 'key space-l5', index: getIndex(5, 13), keyData: getKeyData(getIndex(5, 13)) },
        ...
        { key: KeyDefineEnum.KEY_NUM_0, style: 'key key8 space-l', index: getIndex(5, 16), keyData: getKeyData(getIndex(5, 16)) },
        { key: KeyDefineEnum.KEY_NUM_DOT, style: 'key', index: getIndex(5, 18), keyData: getKeyData(getIndex(5, 18)) },
      ]
    },
  ];
 ...
 /*const keyMatrix_uk = [
  ...
  const state = reactive({
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.Speed_SaveTime, text: keyboard.keyboardDefine?.keyText[KeyDefineEnum.Speed_SaveTime], style: "key", selected: false, tip: 'tip.speed_save_time', type: MatrixTable.WIN },
  ...
  const init = async () => {
    ...
    if (rk_n99.value != undefined && !isInited.value) {
      rk_n99.value.addEventListener(RK_N99_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
  ...
  const destroy = () => {
    if (rk_n99.value != undefined) {
      rk_n99.value.removeEventListener(RK_N99_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
    }
  ...
  const getProfiles = () => {
    if (rk_n99.value != undefined) {
      ...
      if (keyboard.keyboardDefine != undefined) {
        ...
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          ...
          for (index in keyboard.keyboardDefine.keyMatrixLayer) {
            ...
            if (!rk_n99.value.data.keyMatrixs.hasOwnProperty(table)) {
              rk_n99.value.data.keyMatrixs[table] = {};
            }
            rk_n99.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(new Uint8Array(Object.values(profile.value.get(table, layer))).buffer));
      ...
      if (profile.value.profile != undefined) {
        if (rk_n99.value.data.boardProfile != undefined) {
          rk_n99.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
        } else {
          rk_n99.value.data.boardProfile = new BoardProfile(new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer));
        }
      }

      if (profile.value.ledEffect != undefined) {
        if (rk_n99.value.data.ledEffect != undefined) {
          rk_n99.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
        } else {
          rk_n99.value.data.ledEffect = new LedEffect(new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer));
        }
      }

      if (profile.value.ledColors != undefined) {
        if (rk_n99.value.data.ledColors != undefined) {
          rk_n99.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer);
        } else {
          rk_n99.value.data.ledColors = new LedColors(new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer));
        }
      }

      KeyMatrixData.value = rk_n99.value.data.keyMatrixs;
    }
    refresh();
  }
    ...
    if (profile.value != undefined && keyboard.keyboardDefine != undefined) {
      ...
      for (type in keyboard.keyboardDefine.keyMatrixTable) {
        ...
        for (index in keyboard.keyboardDefine.keyMatrixLayer) {
          ...
          await rk_n99.value?.setKeyMatrix(layer, table, 0);
        }
      }
      refresh()
    }

    if (profile.value?.profile != undefined && rk_n99.value != undefined && rk_n99.value.data.boardProfile != undefined) {
      rk_n99.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
    }
    if (profile.value?.ledEffect != undefined && rk_n99.value != undefined && rk_n99.value.data.ledEffect != undefined) {
      rk_n99.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
    }
    if (profile.value?.ledColors != undefined && rk_n99.value != undefined && rk_n99.value.data.ledColors != null) {
      rk_n99.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value?.ledColors)).buffer);
    }
  ...
  const saveProfile = () => {
    ...
    if (rk_n99.value?.data.boardProfile != undefined) {
      const dataView = new DataView(rk_n99.value?.data.boardProfile.buffer.buffer)
      profile.value?.setProfile(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))
    }
    if (rk_n99.value?.data.ledEffect != undefined) {
      const dataView = new DataView(rk_n99.value?.data.ledEffect.buffer.buffer)
      profile.value?.setledEffect(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))

    }
    if (rk_n99.value?.data.ledColors != undefined) {
      const dataView = new DataView(rk_n99.value?.data.ledColors.buffer.buffer) 
  ...
  const setToFactory = async () => {
    if (rk_n99.value != undefined) {
      ...
      await rk_n99.value.setFactory();
      storage.clear();
      getProfiles();
      await rk_n99.value.setProfile(0);
      if (keyboard.keyboardDefine != undefined) {
        let index: any, type: any;
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          let table = keyboard.keyboardDefine.keyMatrixTable[type];
  ...
  const keySetMacro = (index: number) => {
    ...
    if (rk_n99.value != undefined && macros.value == undefined) {
      macros.value = rk_n99.value.data.macros;
    }
