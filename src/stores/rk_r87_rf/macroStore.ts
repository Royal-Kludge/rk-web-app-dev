...
import { Macro, Macros, Action, ActionType } from '@/keyboard/beiying/rk_r87_rf/macros';
...
export const useMacroStore = defineStore("macrostore", () => {
  ...
  const init = async () => {
    ...
    if (rk_r87_rf.value != undefined && !isInited.value) {
      rk_r87_rf.value.addEventListener(RK_R87_RF_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_r87_rf.value != undefined) {
      rk_r87_rf.value.removeEventListener(RK_R87_RF_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
  ...
  const setMacroData = async () => {
    if (rk_r87_rf.value != undefined) {
      await rk_r87_rf.value.setMacros();
    }
  }
