...
export const uselightStore = defineStore('lightinfo', () => {
    ...
    const init = async () => {
        ...
        if (rk_r87_rf.value != undefined && !isInited.value) {
            rk_r87_rf.value.addEventListener(RK_R87_RF_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_r87_rf.value.addEventListener(RK_R87_RF_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_r87_rf.value.addEventListener(RK_R87_RF_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_r87_rf.value.data.boardProfile
            ledEffect.value = rk_r87_rf.value.data.ledEffect
            ledColors.value = rk_r87_rf.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_r87_rf.value != undefined) {
            rk_r87_rf.value.removeEventListener(RK_R87_RF_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_r87_rf.value.removeEventListener(RK_R87_RF_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_r87_rf.value.removeEventListener(RK_R87_RF_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_r87_rf.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_r87_rf.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_r87_rf.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_r87_rf.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_r87_rf.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        // if (rk_r87_rf.value != undefined) {
        //     profile.value = rk_r87_rf.value.data.profile
        //     ledEffect.value = rk_r87_rf.value.data.ledEffect
        //     ledColors.value = rk_r87_rf.value.data.ledColors
        // }
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_r87_rf.value?.setLedColors(profileIndex.value);
            }
            // } else if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            //     if (ledEffect.value != undefined) {
            //         await rk_r87_rf.value?.setLedEffect(profileIndex.value);
            //     }
        }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_r87_rf.value != undefined) {
            ...
            await rk_r87_rf.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
