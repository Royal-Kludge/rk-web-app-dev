...
import { KeyMatrix } from '@/keyboard/beiying/rk_r87_rf/keyMatrix';
import { Action, Macro, Macros } from '@/keyboard/beiying/rk_r87_rf/macros';
import { Profile, ps } from '@/keyboard/beiying/rk_r87_rf/profiles';
import { KeyCodeMap } from '@/common/keyCode_r87_rf'
import fileSaver from "file-saver";
import { ElMessage } from 'element-plus'

import { storage } from '@/common/storage';
import { BoardProfile, FieldEnum, PROFILE_DEFAULT_DATA } from '@/keyboard/beiying/rk_r87_rf/boardProfile';
...
export const useKeyStore = defineStore('keyinfo', () => {
  ...
  const init = async () => {
    ...
    if (rk_r87_rf.value == undefined) {
      ...
      //await rk_r87_rf.value?.setKeyMatrix(layer, MatrixTable.WIN, 0);
      //await rk_r87_rf.value?.setProfile(0);
      //await rk_r87_rf.value?.setLedEffect(0);
      //await rk_r87_rf.value?.setLedColors(0);
    }

    if (rk_r87_rf.value != undefined && !isInited.value) {
      rk_r87_rf.value.addEventListener(RK_R87_RF_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
      macros.value = rk_r87_rf.value.data.macros;
      isInited.value = true;
    }
  };
  ...
  const destroy = () => {
    if (rk_r87_rf.value != undefined) {
      rk_r87_rf.value.removeEventListener(RK_R87_RF_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
    }
      ...
      if (keyboard.keyboardDefine != undefined) {
        ...
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          ...
          for (index in keyboard.keyboardDefine.keyMatrixLayer) {
            ...
            if (!rk_r87_rf.value.data.keyMatrixs.hasOwnProperty(table)) {
              rk_r87_rf.value.data.keyMatrixs[table] = {};
            }
            rk_r87_rf.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(new Uint8Array(Object.values(profile.value.get(table, layer))).buffer));
            //rk_r87_rf.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(profile.value.get(table, layer).buffer));
          }
        }
      }

      if (profile.value.profile != undefined) {
        if (rk_r87_rf.value.data.boardProfile != undefined) {
          rk_r87_rf.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
        } else {
          rk_r87_rf.value.data.boardProfile = new BoardProfile(new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer));
        }
      }

      if (profile.value.ledEffect != undefined) {
        if (rk_r87_rf.value.data.ledEffect != undefined) {
          rk_r87_rf.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
        } else {
          rk_r87_rf.value.data.ledEffect = new LedEffect(new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer));
        }
      }

      if (profile.value.ledColors != undefined) {
        if (rk_r87_rf.value.data.ledColors != undefined) {
          rk_r87_rf.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer);
        } else {
          rk_r87_rf.value.data.ledColors = new LedColors(new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer));
        }
      }

      KeyMatrixData.value = rk_r87_rf.value.data.keyMatrixs;
    }
    refresh();
  }
    ...
    if (profile.value != undefined && keyboard.keyboardDefine != undefined) {
      ...
      for (type in keyboard.keyboardDefine.keyMatrixTable) {
        ...
        for (index in keyboard.keyboardDefine.keyMatrixLayer) {
          ...
          await rk_r87_rf.value?.setKeyMatrix(layer, table, 0);
        }
      }
      refresh()
    }

    if (profile.value?.profile != undefined && rk_r87_rf.value != undefined && rk_r87_rf.value.data.boardProfile != undefined) {
      rk_r87_rf.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
    }
    if (profile.value?.ledEffect != undefined && rk_r87_rf.value != undefined && rk_r87_rf.value.data.ledEffect != undefined) {
      rk_r87_rf.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
    }
    if (profile.value?.ledColors != undefined && rk_r87_rf.value != undefined && rk_r87_rf.value.data.ledColors != null) {
      rk_r87_rf.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value?.ledColors)).buffer);
    }
  ...
  const saveProfile = () => {
    ...
    if (rk_r87_rf.value?.data.boardProfile != undefined) {
      const dataView = new DataView(rk_r87_rf.value?.data.boardProfile.buffer.buffer)
      profile.value?.setProfile(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))
    }
    if (rk_r87_rf.value?.data.ledEffect != undefined) {
      const dataView = new DataView(rk_r87_rf.value?.data.ledEffect.buffer.buffer)
      profile.value?.setledEffect(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))

    }
    if (rk_r87_rf.value?.data.ledColors != undefined) {
      const dataView = new DataView(rk_r87_rf.value?.data.ledColors.buffer.buffer)
  ...
  const newProfile = () => {
    ...
    // if (rk_r87_rf.value != undefined && rk_r87_rf.value.data.boardProfile != undefined) {
    //   tm.profile = new Uint8Array(PROFILE_DEFAULT_DATA.buffer, 0, PROFILE_DEFAULT_DATA.buffer.byteLength);
    // }

    // if (rk_r87_rf.value != undefined && rk_r87_rf.value.data.ledEffect != undefined) {
    //   tm.ledEffect = new Uint8Array(rk_r87_rf.value.data.ledEffect.buffer.buffer, 0, rk_r87_rf.value.data.ledEffect.buffer.byteLength);
    // }
  ...
  const keyMatrixGotten = async (event: any) => {
    ...
    //       ms.add(tm);
    //     }
    //     rk_r87_rf.value.data.macros = ms;
    //     macros.value = rk_r87_rf.value.data.macros;
    //     macro.value = macros.value.get()[0];
    //     refresh();
    //   } else {
    //     await rk_r87_rf.value.getMacros();
    //   }
    // }
  }
  ...
  const setToFactory = async () => {
    if (rk_r87_rf.value != undefined) {
      ...
      await rk_r87_rf.value.setFactory();
      storage.clear();
      getProfiles();
      await rk_r87_rf.value.setProfile(0);
      if (keyboard.keyboardDefine != undefined) {
  ...
  const keyClick = async(index: number) => {
    ...
    if (keyCode != undefined) {
      ...
      await rk_r87_rf.value?.setKeyMatrix(keyMatrixLayer.value, keyMatrixTable.value, 0);
  ...
  const mapping = (keyCode: KeyDefineEnum, table: MatrixTable) => {
    ...
    if (key != undefined) {
      ...
      KeyMatrixData.value[keyMatrixTable.value][keyMatrixLayer.value]?.setKeyMapping(key.index, key.KeyData.keyMappingData);
      rk_r87_rf.value?.setKeyMatrix(keyMatrixLayer.value, keyMatrixTable.value, 0);
      unSelected();
      setUnselected(keyCode);
    }
  ...
  const keySetMacro = (index: number) => {
    ...
    if (rk_r87_rf.value != undefined && macros.value == undefined) {
      macros.value = rk_r87_rf.value.data.macros;
    }
