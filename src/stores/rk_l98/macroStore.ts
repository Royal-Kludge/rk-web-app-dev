...
import { Macro, Macros, Action, ActionType } from '@/keyboard/rk_l98/macros';
...
export const useMacroStore = defineStore("macrostore_rk_l98", () => {
  ...
  const init = async () => {
    ...
    if (rk_l98.value != undefined && !isInited.value) {
      rk_l98.value.addEventListener(RK_L98_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_l98.value != undefined) {
      rk_l98.value.removeEventListener(RK_L98_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
