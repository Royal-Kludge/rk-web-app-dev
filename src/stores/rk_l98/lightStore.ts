...
import { BoardProfile, FieldEnum } from '@/keyboard/rk_l98/boardProfile';
...
import { ps } from '@/keyboard/rk_l98/profiles';

export const useLightStore = defineStore('lightinfo_rk_l98', () => {
    ...
    const init = async () => {
        ...
        if (rk_l98.value != undefined && !isInited.value) {
            rk_l98.value.addEventListener(RK_L98_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_l98.value.addEventListener(RK_L98_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_l98.value.addEventListener(RK_L98_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_l98.value.data.boardProfile
    ...
    const destroy = () => {
        if (rk_l98.value != undefined) {
            rk_l98.value.removeEventListener(RK_L98_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_l98.value.removeEventListener(RK_L98_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_l98.value.removeEventListener(RK_L98_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_l98.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_l98.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_l98.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_l98.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_l98.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_l98.value != undefined) {
            ...
            await rk_l98.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
