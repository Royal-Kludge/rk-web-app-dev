...
import { AdvKeyTypeEnum, MagKeyAdvanceTypeEnum } from "@/keyboard/sparklink/enum";
    ...
    actions: {
        ...
        async saveAdvKey() {
            ...
            switch (this.titleid) {
                case MagKeyAdvanceTypeEnum.DKS:
                    this.addDKS();
                    if (this.key != undefined && this.rk_c61 != undefined) {
                        this.key.keyInfo.isAdvancedKey = true;
                        this.key.keyInfo.advanceKeyType = AdvKeyTypeEnum.DKS;
                        this.key.keyInfo.DKSInfo.DKS[0] = this.advKeyDKS.list[0].key.key;
                        this.key.keyInfo.DKSInfo.DKS[1] = this.advKeyDKS.list[1].key.key;
                        this.key.keyInfo.DKSInfo.DKS[2] = this.advKeyDKS.list[2].key.key;
                        this.key.keyInfo.DKSInfo.DKS[3] = this.advKeyDKS.list[3].key.key;
                        this.key.keyInfo.DKSInfo.TRPS[0] = this.advKeyDKS.list[0].num;
                        this.key.keyInfo.DKSInfo.TRPS[1] = this.advKeyDKS.list[1].num;
                        this.key.keyInfo.DKSInfo.TRPS[2] = this.advKeyDKS.list[2].num;
                        this.key.keyInfo.DKSInfo.TRPS[3] = this.advKeyDKS.list[3].num;
                        await this.rk_c61.setDks([this.key.keyInfo]);
                    }
                    break;
                case MagKeyAdvanceTypeEnum.MT:
                    this.addMT()
        ...
        addMacro() {
            if (this.advanceKeys != undefined && this.advKeyMacro != null && this.isNew == true && this.key != undefined) {
