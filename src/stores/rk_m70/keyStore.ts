...
import { KeyMatrix } from '@/keyboard/beiying/rk_m70/keyMatrix';
import { Action, Macro, Macros } from '@/keyboard/beiying/rk_m70/macros';
import { Profile, ps } from '@/keyboard/beiying/rk_m70/profiles';
import { KeyCodeMap } from '@/common/keyCode'
import fileSaver from "file-saver";
import { ElMessage } from 'element-plus'

import { storage } from '@/common/storage';
import { BoardProfile, FieldEnum, PROFILE_DEFAULT_DATA } from '@/keyboard/beiying/rk_m70/boardProfile';
import { LedEffect } from '@/keyboard/beiying/rk_m70/ledEffect';
import { LedColors } from '@/keyboard/beiying/rk_m70/ledColors';
import { useI18n } from 'vue-i18n';

export const useKeyStore = defineStore('keyinfo_rk_m70', () => {
  ...
  const keyMatrix_us = [
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_PGUP, style: 'key', index: getIndex(3, 16), keyData: getKeyData(getIndex(3, 16)) },
      ]
    },
    {
      line: 5,
  ...
  const keyMatrix_uk = [
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_DEL, style: 'key space-l7', index: getIndex(3, 16), keyData: getKeyData(getIndex(3, 16)) }
      ]
    },
    {
      line: 4,
  ...
  const init = async () => {
    ...
    if (rk_m70.value == undefined) {
      ...
      //await rk_m70.value?.setKeyMatrix(layer, MatrixTable.WIN, 0);
      //await rk_m70.value?.setProfile(0);
      //await rk_m70.value?.setLedEffect(0);
      //await rk_m70.value?.setLedColors(0);
    }

    if (rk_m70.value != undefined && !isInited.value) {
      rk_m70.value.addEventListener(RK_M70_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
      macros.value = rk_m70.value.data.macros;
      isInited.value = true;
    }
  };
  ...
  const destroy = () => {
    if (rk_m70.value != undefined) {
      rk_m70.value.removeEventListener(RK_M70_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
    }
      ...
      if (keyboard.keyboardDefine != undefined) {
        ...
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          ...
          for (index in keyboard.keyboardDefine.keyMatrixLayer) {
            ...
            if (!rk_m70.value.data.keyMatrixs.hasOwnProperty(table)) {
              rk_m70.value.data.keyMatrixs[table] = {};
            }
            rk_m70.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(new Uint8Array(Object.values(profile.value.get(table, layer))).buffer));
            //rk_m70.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(profile.value.get(table, layer).buffer));
          }
        }
      }

      if (profile.value.profile != undefined) {
        if (rk_m70.value.data.boardProfile != undefined) {
          rk_m70.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
        } else {
          rk_m70.value.data.boardProfile = new BoardProfile(new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer));
        }
      }

      if (profile.value.ledEffect != undefined) {
        if (rk_m70.value.data.ledEffect != undefined) {
          rk_m70.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
        } else {
          rk_m70.value.data.ledEffect = new LedEffect(new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer));
        }
      }

      if (profile.value.ledColors != undefined) {
        if (rk_m70.value.data.ledColors != undefined) {
          rk_m70.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer);
        } else {
          rk_m70.value.data.ledColors = new LedColors(new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer));
        }
      }

      KeyMatrixData.value = rk_m70.value.data.keyMatrixs;
    }
    refresh();
  }
    ...
    if (profile.value != undefined && keyboard.keyboardDefine != undefined) {
      ...
      for (type in keyboard.keyboardDefine.keyMatrixTable) {
        ...
        for (index in keyboard.keyboardDefine.keyMatrixLayer) {
          ...
          await rk_m70.value?.setKeyMatrix(layer, table, 0);
        }
      }
      refresh()
    }

    if (profile.value?.profile != undefined && rk_m70.value != undefined && rk_m70.value.data.boardProfile != undefined) {
      rk_m70.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
    }
    if (profile.value?.ledEffect != undefined && rk_m70.value != undefined && rk_m70.value.data.ledEffect != undefined) {
      rk_m70.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
    }
    if (profile.value?.ledColors != undefined && rk_m70.value != undefined && rk_m70.value.data.ledColors != null) {
      rk_m70.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value?.ledColors)).buffer);
    }
  ...
  const saveProfile = () => {
    ...
    if (rk_m70.value?.data.boardProfile != undefined) {
      const dataView = new DataView(rk_m70.value?.data.boardProfile.buffer.buffer)
      profile.value?.setProfile(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))
    }
    if (rk_m70.value?.data.ledEffect != undefined) {
      const dataView = new DataView(rk_m70.value?.data.ledEffect.buffer.buffer)
      profile.value?.setledEffect(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))

    }
    if (rk_m70.value?.data.ledColors != undefined) {
      const dataView = new DataView(rk_m70.value?.data.ledColors.buffer.buffer)
  ...
  const newProfile = () => {
    ...
    // if (rk_m70.value != undefined && rk_m70.value.data.boardProfile != undefined) {
    //   tm.profile = new Uint8Array(PROFILE_DEFAULT_DATA.buffer, 0, PROFILE_DEFAULT_DATA.buffer.byteLength);
    // }

    // if (rk_m70.value != undefined && rk_m70.value.data.ledEffect != undefined) {
    //   tm.ledEffect = new Uint8Array(rk_m70.value.data.ledEffect.buffer.buffer, 0, rk_m70.value.data.ledEffect.buffer.byteLength);
    // }
  ...
  const keyMatrixGotten = async (event: any) => {
    ...
    //       ms.add(tm);
    //     }
    //     rk_m70.value.data.macros = ms;
    //     macros.value = rk_m70.value.data.macros;
    //     macro.value = macros.value.get()[0];
    //     refresh();
    //   } else {
    //     await rk_m70.value.getMacros();
    //   }
    // }
  }
  ...
  const keyClick = async(index: number) => {
    ...
    if (keyCode != undefined) {
      ...
      await rk_m70.value?.setKeyMatrix(keyMatrixLayer.value, keyMatrixTable.value, 0);
  ...
  const setKeyCode = (index: number, keyCode: KeyDefineEnum) => {
    ...
    KeyMatrixData.value[keyMatrixTable.value][keyMatrixLayer.value]?.setKeyMapping(key.index, key.KeyData.keyMappingData);
    rk_m70.value?.setKeyMatrix(keyMatrixLayer.value, keyMatrixTable.value, 0);
  ...
  const keySetMacro = (index: number) => {
    ...
    if (rk_m70.value != undefined && macros.value == undefined) {
      macros.value = rk_m70.value.data.macros;
    }
