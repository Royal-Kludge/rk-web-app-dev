...
import { Macro, Macros, Action, ActionType } from '@/keyboard/beiying/rk_m70/macros';
...
export const useMacroStore = defineStore("macrostore_rk_m70", () => {
  ...
  const init = async () => {
    ...
    if (rk_m70.value != undefined && !isInited.value) {
      rk_m70.value.addEventListener(RK_M70_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_m70.value != undefined) {
      rk_m70.value.removeEventListener(RK_M70_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
