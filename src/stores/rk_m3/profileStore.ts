...
import { KeyTable } from "@/mouse/beiying/rk_m3/keyTable";
import { Profile, ps } from '@/mouse/beiying/rk_m3/profiles';
import { ConnectionStatusEnum } from "@/device/enum";

export const useProfileStore = defineStore('profile_rk_m3', () => {
        ...
        if (rk_m3.value != undefined && profile.value != undefined) {
            if (profile.value.keyLayout)
                if (profile.value.keyTable != undefined) {
                    if (rk_m3.value.data.keys != undefined) {
                        rk_m3.value.data.keys.buffer = new DataView(profile.value.keyTable.buffer);
                    } else {
                        rk_m3.value.data.keys = new KeyTable(new DataView(profile.value.keyTable.buffer));
                    }
                }
            ...
            if (profile.value.ledTable != undefined) {
                if (rk_m3.value.data.led != undefined) {
                    rk_m3.value.data.led.buffer = new DataView(profile.value.ledTable.buffer);
                } else {
