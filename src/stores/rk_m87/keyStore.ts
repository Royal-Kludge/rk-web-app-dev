...
import { KeyMatrix } from '@/keyboard/beiying/rk_m87/keyMatrix';
import { Action, Macro, Macros } from '@/keyboard/beiying/rk_m87/macros';
import { Profile, ps } from '@/keyboard/beiying/rk_m87/profiles';
import { KeyCodeMap } from '@/common/keyCode_m87'
import fileSaver from "file-saver";
import { ElMessage } from 'element-plus'

import { storage } from '@/common/storage';
import { BoardProfile, FieldEnum, PROFILE_DEFAULT_DATA } from '@/keyboard/beiying/rk_m87/boardProfile';
import { LedEffect } from '@/keyboard/beiying/rk_m87/ledEffect';
import { LedColors } from '@/keyboard/beiying/rk_m87/ledColors';
import { useI18n } from 'vue-i18n';

export const useKeyStore = defineStore('keyinfo_rk_m87', () => {
  ...
  const init = async () => {
    ...
    if (rk_m87.value != undefined && !isInited.value) {
      rk_m87.value.addEventListener(RK_M87_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
  ...
  const destroy = () => {
    if (rk_m87.value != undefined) {
      rk_m87.value.removeEventListener(RK_M87_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
    }
  ...
  const getProfiles = () => {
    if (rk_m87.value != undefined) {
      ...
      if (keyboard.keyboardDefine != undefined) {
        ...
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          ...
          for (index in keyboard.keyboardDefine.keyMatrixLayer) {
            ...
            if (!rk_m87.value.data.keyMatrixs.hasOwnProperty(table)) {
              rk_m87.value.data.keyMatrixs[table] = {};
            }
            rk_m87.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(new Uint8Array(Object.values(profile.value.get(table, layer))).buffer));
      ...
      if (profile.value.profile != undefined) {
        if (rk_m87.value.data.boardProfile != undefined) {
          rk_m87.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
        } else {
          rk_m87.value.data.boardProfile = new BoardProfile(new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer));
        }
      }

      if (profile.value.ledEffect != undefined) {
        if (rk_m87.value.data.ledEffect != undefined) {
          rk_m87.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
        } else {
          rk_m87.value.data.ledEffect = new LedEffect(new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer));
        }
      }

      if (profile.value.ledColors != undefined) {
        if (rk_m87.value.data.ledColors != undefined) {
          rk_m87.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer);
        } else {
          rk_m87.value.data.ledColors = new LedColors(new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer));
        }
      }

      KeyMatrixData.value = rk_m87.value.data.keyMatrixs;
    }
    refresh();
  }
  ...
  const clickProfile = async (obj: Profile) => {
    ...
    if (profile.value?.profile != undefined && rk_m87.value != undefined && rk_m87.value.data.boardProfile != undefined) {
      rk_m87.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
    }
    if (profile.value?.ledEffect != undefined && rk_m87.value != undefined && rk_m87.value.data.ledEffect != undefined) {
      rk_m87.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
    }
    if (profile.value?.ledColors != undefined && rk_m87.value != undefined && rk_m87.value.data.ledColors != null) {
      rk_m87.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value?.ledColors)).buffer);
    }
  ...
  const saveProfile = () => {
    ...
    if (rk_m87.value?.data.boardProfile != undefined) {
      const dataView = new DataView(rk_m87.value?.data.boardProfile.buffer.buffer)
      profile.value?.setProfile(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))
    }
    if (rk_m87.value?.data.ledEffect != undefined) {
      const dataView = new DataView(rk_m87.value?.data.ledEffect.buffer.buffer)
      profile.value?.setledEffect(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))

    }
    if (rk_m87.value?.data.ledColors != undefined) {
      const dataView = new DataView(rk_m87.value?.data.ledColors.buffer.buffer)
  ...
  const keySetMacro = (index: number) => {
    ...
    if (rk_m87.value != undefined && macros.value == undefined) {
      macros.value = rk_m87.value.data.macros;
    }
