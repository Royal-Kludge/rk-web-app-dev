...
import { BoardProfile, FieldEnum } from '@/keyboard/beiying/rk_m87/boardProfile';
...
import { ps } from '@/keyboard/beiying/rk_m87/profiles';

export const uselightStore = defineStore('lightinfo_rk_m87', () => {
    ...
    const init = async () => {
        ...
        if (rk_m87.value != undefined && !isInited.value) {
            rk_m87.value.addEventListener(RK_M87_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_m87.value.addEventListener(RK_M87_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_m87.value.addEventListener(RK_M87_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_m87.value.data.boardProfile
    ...
    const destroy = () => {
        if (rk_m87.value != undefined) {
            rk_m87.value.removeEventListener(RK_M87_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_m87.value.removeEventListener(RK_M87_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_m87.value.removeEventListener(RK_M87_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_m87.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_m87.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_m87.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_m87.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_m87.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_m87.value != undefined) {
            ...
            await rk_m87.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
