...
import { Macro, Macros, Action, ActionType } from '@/mouse/beiying/rk_k3/macros';
...
export const useMacroStore = defineStore("macrostore_rk_k3", () => {
    ...
    const init = async () => {
        ...
        if (rk_k3.value != undefined && !isInited.value) {
            rk_k3.value.addEventListener(RK_MOUSE_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
            ...
            if (tmp != null) {
                ...
                rk_k3.value.data.macros = ms;
                macros.value = rk_k3.value.data.macros;
    ...
    const destroy = () => {
        if (rk_k3.value != undefined) {
            rk_k3.value.removeEventListener(RK_MOUSE_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
        }
    ...
    const getMacroData = async () => {
        if (rk_k3.value != undefined) {
            await rk_k3.value.getMacros();
        }
    }

    const setMacroData = async () => {
        if (rk_k3.value != undefined) {
            await rk_k3.value.setMacros();
        }
    }
