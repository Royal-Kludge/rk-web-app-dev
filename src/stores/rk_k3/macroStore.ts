...
export const useMacroStore = defineStore("macrostore_rk_k3", () => {
    ...
    const init = async () => {
        ...
        if (rk_k3.value != undefined && !isInited.value) {
            rk_k3.value.addEventListener(RK_K3_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    ...
    const destroy = () => {
        if (rk_k3.value != undefined) {
            rk_k3.value.removeEventListener(RK_K3_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
        }
