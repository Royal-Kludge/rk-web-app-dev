...
export const useKeyStore = defineStore('keyinfo_rk_l75', () => {
  ...
  const keyMatrix_us = [
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_F5, style: 'key space-l', index: getIndex(0, 6), keyData: getKeyData(getIndex(0, 6)) },
        ...
        { key: KeyDefineEnum.KEY_F9, style: 'key space-l', index: getIndex(0, 10), keyData: getKeyData(getIndex(0, 10)) },
    ...
    },
    {
      ...
      keys: [
        ...
        { key: KeyDefineEnum.KEY_DEL, style: 'key space-l', index: getIndex(1, 16), keyData: getKeyData(getIndex(1, 16)) },
      ]
    },
    {
      line: 3,
  ...
  const init = async () => {
    ...
    if (rk_l75.value == undefined) {
      ...
      //await rk_l75.value?.setKeyMatrix(layer, MatrixTable.WIN, 0);
      //await rk_l75.value?.setProfile(0);
      //await rk_l75.value?.setLedEffect(0);
      //await rk_l75.value?.setLedColors(0);
    }

    if (rk_l75.value != undefined && !isInited.value) {
      rk_l75.value.addEventListener(RK_L75_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
  ...
  const destroy = () => {
    if (rk_l75.value != undefined) {
      rk_l75.value.removeEventListener(RK_L75_EVENT_DEFINE.OnKeyMatrixGotten, keyMatrixGotten, false);
    }
      ...
      if (keyboard.keyboardDefine != undefined) {
        ...
        for (type in keyboard.keyboardDefine.keyMatrixTable) {
          ...
          for (index in keyboard.keyboardDefine.keyMatrixLayer) {
            ...
            if (!rk_l75.value.data.keyMatrixs.hasOwnProperty(table)) {
              rk_l75.value.data.keyMatrixs[table] = {};
            }
            rk_l75.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(new Uint8Array(Object.values(profile.value.get(table, layer))).buffer));
            //rk_l75.value.data.keyMatrixs[table][layer] = new KeyMatrix(new DataView(profile.value.get(table, layer).buffer));
          }
        }
      }

      if (profile.value.profile != undefined) {
        if (rk_l75.value.data.boardProfile != undefined) {
          rk_l75.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
        } else {
          rk_l75.value.data.boardProfile = new BoardProfile(new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer));
        }
      }

      if (profile.value.ledEffect != undefined) {
        if (rk_l75.value.data.ledEffect != undefined) {
          rk_l75.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
        } else {
          rk_l75.value.data.ledEffect = new LedEffect(new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer));
        }
      }

      if (profile.value.ledColors != undefined) {
        if (rk_l75.value.data.ledColors != undefined) {
          rk_l75.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer);
        } else {
          rk_l75.value.data.ledColors = new LedColors(new DataView(new Uint8Array(Object.values(profile.value.ledColors)).buffer));
        }
      }

      KeyMatrixData.value = rk_l75.value.data.keyMatrixs;
    }
    refresh();
  }
    ...
    if (profile.value != undefined && keyboard.keyboardDefine != undefined) {
      ...
      for (type in keyboard.keyboardDefine.keyMatrixTable) {
        ...
        for (index in keyboard.keyboardDefine.keyMatrixLayer) {
          ...
          await rk_l75.value?.setKeyMatrix(layer, table, 0);
        }
      }
      refresh()
    }

    if (profile.value?.profile != undefined && rk_l75.value != undefined && rk_l75.value.data.boardProfile != undefined) {
      rk_l75.value.data.boardProfile.buffer = new DataView(new Uint8Array(Object.values(profile.value.profile)).buffer);
    }
    if (profile.value?.ledEffect != undefined && rk_l75.value != undefined && rk_l75.value.data.ledEffect != undefined) {
      rk_l75.value.data.ledEffect.buffer = new DataView(new Uint8Array(Object.values(profile.value.ledEffect)).buffer);
    }
    if (profile.value?.ledColors != undefined && rk_l75.value != undefined && rk_l75.value.data.ledColors != null) {
      rk_l75.value.data.ledColors.buffer = new DataView(new Uint8Array(Object.values(profile.value?.ledColors)).buffer);
    }
  ...
  const saveProfile = () => {
    ...
    if (rk_l75.value?.data.boardProfile != undefined) {
      const dataView = new DataView(rk_l75.value?.data.boardProfile.buffer.buffer)
      profile.value?.setProfile(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))
    }
    if (rk_l75.value?.data.ledEffect != undefined) {
      const dataView = new DataView(rk_l75.value?.data.ledEffect.buffer.buffer)
      profile.value?.setledEffect(new Uint8Array(dataView.buffer, dataView.byteOffset, dataView.byteLength))

    }
    if (rk_l75.value?.data.ledColors != undefined) {
      const dataView = new DataView(rk_l75.value?.data.ledColors.buffer.buffer)
  ...
  const newProfile = () => {
    ...
    // if (rk_l75.value != undefined && rk_l75.value.data.boardProfile != undefined) {
    //   tm.profile = new Uint8Array(PROFILE_DEFAULT_DATA.buffer, 0, PROFILE_DEFAULT_DATA.buffer.byteLength);
    // }

    // if (rk_l75.value != undefined && rk_l75.value.data.ledEffect != undefined) {
    //   tm.ledEffect = new Uint8Array(rk_l75.value.data.ledEffect.buffer.buffer, 0, rk_l75.value.data.ledEffect.buffer.byteLength);
    // }
  ...
  const keySetToDefaultAll = async () => {
    ...
    if (keyboard.keyboardDefine == undefined) return;
    ...
    for (index in state.keyState) {
      ...
      let key: KeyTableData = {
        keyStr: keyboard.keyboardDefine?.keyText[code],
        keyCode: code,
        index: index,
  ...
  const keySetToDefault = (index: number) => {
    if (state.keyState.length <= 0 || index >= 999 || keyboard.keyboardDefine == undefined) {
      return '';
    }
  ...
  const keySetMacro = (index: number) => {
    ...
    if (rk_l75.value != undefined && macros.value == undefined) {
      macros.value = rk_l75.value.data.macros;
    }
