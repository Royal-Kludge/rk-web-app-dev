...
import { BoardProfile, FieldEnum } from '@/keyboard/beiying/rk_l75/boardProfile';
...
import { ps } from '@/keyboard/beiying/rk_l75/profiles';
...
export const uselightStore = defineStore('lightinfo_rk_l75', () => {
    ...
    const init = async () => {
        ...
        if (rk_l75.value != undefined && !isInited.value) {
            rk_l75.value.addEventListener(RK_L75_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_l75.value.addEventListener(RK_L75_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_l75.value.addEventListener(RK_L75_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
            boardProfile.value = rk_l75.value.data.boardProfile
            ledEffect.value = rk_l75.value.data.ledEffect
            ledColors.value = rk_l75.value.data.ledColors
            isInited.value = true;
        }
    ...
    const destroy = () => {
        if (rk_l75.value != undefined) {
            rk_l75.value.removeEventListener(RK_L75_EVENT_DEFINE.OnProfileGotten, profileGotten, false);
            rk_l75.value.removeEventListener(RK_L75_EVENT_DEFINE.OnLedEffectGotten, ledEffectGotten, false);
            rk_l75.value.removeEventListener(RK_L75_EVENT_DEFINE.OnLedColorsGotten, ledColorsGotten, false);
        }
    ...
    const DebounceChanged = (mode: number) => {
        state.debounce = mode;
        if (boardProfile.value != undefined && rk_l75.value != undefined) {
            boardProfile.value.setFieldValue(FieldEnum.Debounce, mode);
            rk_l75.value.setProfile(profileIndex.value);
        }
    };
    ...
    const getLightData = async () => {
        await rk_l75.value?.getProfile(profileIndex.value);
    }
    ...
    const profileGotten = async (event: any) => {
        ...
        await rk_l75.value?.getLedEffect(profileIndex.value);
        setProfile()
    };

    const ledEffectGotten = async (event: any) => {
        ledEffect.value = event.detail as LedEffect;
        await rk_l75.value?.getLedColors(profileIndex.value);
        setledEffect()
    };
    ...
    const refresh = async () => {
        // if (rk_l75.value != undefined) {
        //     profile.value = rk_l75.value.data.profile
        //     ledEffect.value = rk_l75.value.data.ledEffect
        //     ledColors.value = rk_l75.value.data.ledColors
        // }
        ...
        if (state.lightProps.light == LightEffectEnum.SelfDefine) {
            if (ledColors.value != undefined) {
                await rk_l75.value?.setLedColors(profileIndex.value);
            }
            // } else if (state.lightProps.light != LightEffectEnum.OFF && state.lightProps.light != LightEffectEnum.Music) {
            //     if (ledEffect.value != undefined) {
            //         await rk_l75.value?.setLedEffect(profileIndex.value);
            //     }
        }
    ...
    const ligtChanged = async () => {
        if (boardProfile.value != undefined && rk_l75.value != undefined) {
            ...
            await rk_l75.value.setProfile(profileIndex.value);
            await refresh();
        }
    };
