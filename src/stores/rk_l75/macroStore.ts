...
import { Macro, Macros, Action, ActionType } from '@/keyboard/beiying/rk_l75/macros';
...
export const useMacroStore = defineStore("macrostore_rk_l75", () => {
  ...
  const init = async () => {
    ...
    if (rk_l75.value != undefined && !isInited.value) {
      rk_l75.value.addEventListener(RK_L75_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
  ...
  const destroy = () => {
    if (rk_l75.value != undefined) {
      rk_l75.value.removeEventListener(RK_L75_EVENT_DEFINE.OnMacrosGotten, macroGotten, false);
    }
